...
function getFactoringStatusDisplayName(
  status: FactoringStatus | undefined | null
): string {
  if (!status || status === FactoringStatus.NONE) return "";
  switch (status) {
    case FactoringStatus.REQUESTED:
      return "Factoring Requested";
    case FactoringStatus.BUYER_ACCEPTED:
      return "Buyer Accepted";
    case FactoringStatus.BUYER_REJECTED:
      return "Buyer Rejected";
    case FactoringStatus.PENDING_FINANCING:
      return "Pending Financing";
    case FactoringStatus.FINANCED:
      return "Financed";
    case FactoringStatus.REPAID:
      return "Repaid to Financier";
    default:
...
export default async function DashboardPage() {
  ...
  if (profileError || !profileData) {
    if (profileError && (profileError as any).message) {
      console.error(
        "Dashboard: Error fetching profile - Supabase message:",
      ...
      console.error(
        `Dashboard: Error fetching profile - No profile data found for user ID: ${user.id}. Raw profileError:`,
        profileError
      );
    } else {
      console.error(
        "Dashboard: Error fetching profile - An unspecified issue occurred. Raw profileError:",
        profileError,
        "ProfileData exists:",
        !!profileData
      );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <p className="text-muted-foreground">
          Could not load your user profile. Please try again later.
        </p>
      </div>
    );
  }
  ...
  if (profile.role === UserRole.MSME) {
    ...
    invoicesQuery = supabase
      ...
      .select(selectString)
      .in("factoring_status", [
        FactoringStatus.BUYER_ACCEPTED,
        FactoringStatus.PENDING_FINANCING,
        FactoringStatus.FINANCED,
      ])
      .order("created_at", { ascending: false });
  } else {
    console.warn(
      `Dashboard: User ${user.id} has role '${profile.role}' and email '${user.email}', which doesn't match MSME, BUYER with email, or FINANCIER for invoice fetching. Returning empty set.`
    );
  ...
  if (invoicesError) {
    const errorMessage =
      ...
      "No specific error message. Error object might be empty or not an instance of Error.";
    const userContext = `User ID: ${user.id}, Role: ${
      profile.role
    }, Email for query (if buyer): ${
      profile.role === UserRole.BUYER ? user.email : "N/A"
    }.`;
    let joinDebugInfo = "";
    ...
    console.error(
      `Error fetching invoices: ${errorMessage}.${joinDebugInfo} ${userContext}. Raw error object:`,
      invoicesError
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <p className="text-muted-foreground">
          Could not load your invoice data. Please try again later.
        </p>
  ...
  const invoices: Invoice[] = (invoicesData || []).map((inv: any) => ({
    ...
    sellerName:
      (profile.role === UserRole.BUYER ||
        profile.role === UserRole.FINANCIER) &&
      inv.profiles
        ? inv.profiles.full_name
        : profile.role === UserRole.MSME
        ? profile.full_name
        : "N/A",
    clientHistory: inv.client_history, // Add missing required property
    industryStandards: inv.industry_standards, // Add missing required property
  ...
  const draftInvoices = invoices.filter(
    (inv) => inv.status === InvoiceStatus.DRAFT && isMSME
  );
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Across {outstandingInvoices.length} invoices
              </p>
            </CardContent>
          </Card>
          <Card>
        ...
        <Card>
          <CardHeader className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>
                These are invoices where you are listed as the client. You can
                review factoring requests here.
              </CardDescription>
            </div>
          </CardHeader>
        </Card>
        ...
        <Card>
          <CardHeader className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>
                Invoices accepted by buyers or pending financing bids. Review
                details and place your bids.
              </CardDescription>
            </div>
          </CardHeader>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {invoices.length > 0
              ? `A list of ${
                  isMSME ? "your recent" : isBuyer ? "your" : "available"
                } invoices.`
              : "No invoices found."}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>
                    {isMSME ? "Client (Buyer)" : "Seller (MSME)"}
                  </TableHead>
                          ...
                          <Badge
                            variant={
                              ...
                              FactoringStatus.REQUESTED
                                ...
                                ? "default"
                                : invoice.factoring_status ===
                                  FactoringStatus.BUYER_REJECTED
                                ? "destructive"
                                : invoice.factoring_status ===
                                  FactoringStatus.PENDING_FINANCING
                                ? "secondary"
